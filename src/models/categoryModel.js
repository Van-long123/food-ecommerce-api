...
const CATEGORY_COLLECTION_SCHEMA = Joi.object({
  ...
  type: Joi.string().valid(...Object.values(CATEGORY_TYPES)).required(),
  description: Joi.string().allow('').default(''),
  thumbnail: Joi.string().allow('').default(''),
  bannerImage: Joi.string().allow('').default(''),
  badgeText: Joi.string().allow('').default(''),
  status: Joi.string().valid(...Object.values(CATEGORY_STATUSES)).default(CATEGORY_STATUSES.ACTIVE),
  featured: Joi.boolean().default(false),
  position: Joi.number().integer().default(0),
  parent_id: Joi.alternatives().try(Joi.string(), Joi.allow(null)).default(null),
...
const createNew = async (data) => {
  try {
    ...
    const result = await GET_DB().collection(CATEGORY_COLLECTION_NAME).insertOne(persistData)
    return result
  } catch (error) {
...
const findOneById = async (id) => {
  try {
    return await GET_DB().collection(CATEGORY_COLLECTION_NAME).findOne({ _id: new ObjectId(id) })
  } catch (error) {
    throw new Error(error)
  }
}
...
const findOneBySlug = async (slug) => {
  try {
    return await GET_DB().collection(CATEGORY_COLLECTION_NAME).findOne({ slug, deleted: false })
  } catch (error) {
    throw new Error(error)
  }
}
...
const getList = async ({ queryConditions = [], page = 1, limit = 10, sort = { position: 1 } }) => {
  try {
    const query = await GET_DB().collection(CATEGORY_COLLECTION_NAME).aggregate([
  ...
  try {
    return await GET_DB().collection(CATEGORY_COLLECTION_NAME)
      ...
      .project({
        ...
        thumbnail: 1, bannerImage: 1, badgeText: 1,
        parent_id: 1, featured: 1, position: 1
      })
      .toArray()
  } catch (error) {
    throw new Error(error)

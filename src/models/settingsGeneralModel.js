...
const validateBeforeCreate = async (data) => {
  return SETTINGS_GENERAL_COLLECTION_SCHEMA.validateAsync(data, {
    abortEarly: false,
  });
};
...
const createNew = async (data) => {
  try {
    ...
    const created = await GET_DB()
      .collection(SETTINGS_GENERAL_COLLECTION_NAME)
      .insertOne(validData);
    return created;
  } catch (error) {
...
const updateSettings = async (updateData) => {
  try {
    ...
    return await GET_DB()
      .collection(SETTINGS_GENERAL_COLLECTION_NAME)
      .findOneAndUpdate(
        {}, // Cập nhật tài liệu cài đặt chung duy nhất
        { $set: cleanUpdateData },
        { returnDocument: "after", upsert: true }, // Cập nhật nếu cài đặt chưa tồn tại.
      );
  } catch (error) {
    throw new Error(error);
  }

...
const createNew = async (data) => {
  try {
    ...
    const result = await GET_DB().collection(ADDRESS_COLLECTION_NAME).insertOne({
      ...validData,
...
const findOneById = async (id) => {
  try {
    const result = await GET_DB().collection(ADDRESS_COLLECTION_NAME).findOne({
      _id: new ObjectId(id)
    })
    return result
  } catch (error) {
...
const unsetDefaultAddresses = async (userId, excludeAddressId = null) => {
  try {
    ...
    await GET_DB().collection(ADDRESS_COLLECTION_NAME).updateMany(
      filter,
      { $set: { default: 0 } }
    )
  } catch (error) {

...
const createNew = async (reqBody, actorId, files = null) => {
  try {
    ...
    let categoryIds = reqBody.category_ids || [];
    ...
    const newArticle = {
      ...
      readTime: parseNum(reqBody.readTime, 0),
      views: parseNum(reqBody.views, 0),
...
const getListAdmin = async (query) => {
  try {
    ...
    const { data, total, stats } = await articleModel.getList({
      queryConditions,
      categoryId,
      page,
      limit,
...
const update = async (id, reqBody, actorId, files = null) => {
  try {
    ...
    if (reqBody.publishedAt)
      updateData.publishedAt = new Date(reqBody.publishedAt);
    if (reqBody.readTime !== undefined)
      updateData.readTime = parseNum(reqBody.readTime, 0);
    if (reqBody.views !== undefined)
      updateData.views = parseNum(reqBody.views, 0);
    if (
    ...
    if (reqBody.primary_category_id === "") {
      updateData.primary_category_id = null;
    }
    ...
    if (reqBody.category_ids !== undefined) {
      ...
      if (!Array.isArray(categoryIds))
        categoryIds = categoryIds ? [categoryIds] : [];
      const primaryId = reqBody.primary_category_id || categoryIds[0] || null;
      await syncCategories(id, categoryIds, primaryId);
    }
...
const addCategory = async (articleId, reqBody) => {
  try {
    ...
    if (reqBody.isPrimary) {
      await articleModel.update(articleId, {
...
const removeCategory = async (articleId, categoryId) => {
  try {
    ...
    await categoryArticleModel.removeOne({
      article_id: articleId,
...
const bulkUpdateStatusAdmin = async ({ article_ids = [], status }) => {
  try {
    if (!Array.isArray(article_ids) || article_ids.length === 0) {
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
        "Danh sách bài viết không hợp lệ!",
      );
    }

    const result = await articleModel.updateManyStatus(article_ids, status);
...
const bulkDeleteAdmin = async ({ article_ids = [] }, actorId) => {
  try {
    if (!Array.isArray(article_ids) || article_ids.length === 0) {
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
    ...
    const result = await articleModel.softDeleteMany(
      article_ids,
      actorId,
      actor.email,
    );

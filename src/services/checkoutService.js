...
const createPayOSCheckout = async (userId, payload) => {
  ...
  try {
    ...
    const paymentLink = await createPayOSPaymentLink({
      ...
      cancelUrl: `${env.BUILD_MODE === 'production' ? env.WEBSITE_DOMAIN_PROD : env.WEBSITE_DOMAIN_DEV}/order/${createdOrderId}`,
      returnUrl: `${env.BUILD_MODE === 'production' ? env.WEBSITE_DOMAIN_PROD : env.WEBSITE_DOMAIN_DEV}/order/${createdOrderId}`,
